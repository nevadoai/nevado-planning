...
2. **No SDK change was made.** Anywhere this document anticipates a change to `@nevadoai/sherpa-core` or `@nevadoai/sherpa-protocol`, none happened. SDK work was explicitly authorised and declined on merit: `'completed'` is a human complete/archive flag, emitting it from the engine loop would break `nevado-sherpa-ide`'s `/resume`, and no session status can separate a session that submitted a result from one that did not. `createdBy` remains forced to the manager's `userId`, correctly — it backs a live ownership check in `renameSession` — so the `m2m:<CN>` and `m2m:bearer` values this document discusses are recorded in the create **request** and discarded by the SDK. They are not a migration path for a per-client ACL predicate, and a shared bearer token could not supply one anyway.
...
**Two options are implemented behind `var.sdlc_ingress_mode`. `token` is the one that runs; `mtls` is retained and inactive.** Everything below about deleting the API Gateway, the M2M client and the shared header secret holds for both — that part of this decision is unchanged.
