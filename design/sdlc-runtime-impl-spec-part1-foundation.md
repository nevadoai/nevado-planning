...
## Implemented, 2026-09-26 — three corrections to this document

The foundation described here was built. Three things in the text below changed during implementation:

1. **Ingress (§0.9.1) now documents two options.** `token` — a bearer token the runtime verifies itself, generated by Terraform with no human step — is what runs. `mtls` is retained and inactive. §0.9.1 has been rewritten in place, including the correction that mTLS was declined on **cost** rather than feasibility.
2. **No SDK release gated any of this, and the SDK change that mattered was declined.** Where this document anticipates a `sherpa-sdk` dependency chain, there was none: the SDLC work was built in `apps/runtime/src/sdlc/` against unchanged **4.0.1**, which Command Center still pins. One SDK release did ship — `sherpa-sdk#227`, released as **v4.1.2** — but it only widened `SessionApplication` to include `'sdlc'` so the runtime's ACL comparison is type-checked instead of cast, and nobody waited on it. `sherpa-sdk#226`, which would have unforced `createdBy` and `application` on create, was **closed unmerged** on merit: `createdBy` backs a live ownership check in `renameSession`, so letting a caller supply it would let a caller answer an authorization question about itself. So `createdBy` remains forced to the manager's `userId`, and the `m2m:<CN>` / `m2m:bearer` values this document discusses are recorded in the create **request** and discarded by the SDK. They are not a migration path for a per-client ACL predicate, and a shared bearer token could not supply one anyway. Making the engine emit `'completed'` was declined for its own reasons — it is a human complete/archive flag, and emitting it would break `nevado-sherpa-ide`'s `/resume`.
3. **`GET /v1/sdlc/sessions/{id}` is not authoritative about outcome.** It is authoritative about a session's **checkout identity** and its **liveness** only. Outcome arrives as a published event and is recorded on the cycle. Any fallback here that resolves a result from the session record is dead: the record carries no commit for an engineering session, because only the QA dispatch pins one.

**Line numbers and SHAs below are as verified on 2026-09-23 at `develop` = `938be378`** and have moved considerably since. Treat them as a guide to the right file, not as current coordinates.
...
- **SDK target is 4.0.2 patched from 4.0.1**, not HEAD.
- **New Task 2.2b** builds `GET` and `cancel`, which no earlier task did.
...
|---|---|---|---|
| Command Center | `/Users/jwoolley/dev/nevadoai/command-center` | Orchestrator, consumer Lambda, Terraform, frontend | GitHub Actions (see §0.3) |
| Sherpa SDK | `/Users/jwoolley/dev/nevadoai/sherpa-sdk` | `packages/protocol`, `packages/core` (AgentEngine, tools, PlatformAdapter), `packages/web` | npm publish to `npm.pkg.github.com` |
| Sherpa runtime server | `/Users/jwoolley/dev/nevadoai/nevado-sherpa-tui` | `apps/runtime/src` — the Fastify HTTP+WS server that serves `/sessions` and `/ws` | see §0.3 / Phase 1 Task 1.6 |

`nevado-sherpa-tui` is the repo the architecture doc calls `nevado-sherpa-tui`. There is a symlink `coding-tools/nevado-sherpa-tui -> ../nevado-sherpa-tui`. `/Users/jwoolley/dev/nevadoai/nevado-sherpa-ide` is the VS Code extension and is **not** relevant.

**Warning:** `nevado-sherpa-tui/.claude/worktrees/*` contains full stale copies of the tree. Grep results under that path are duplicates, not real code. Always confirm a hit is under `nevado-sherpa-tui/apps/` or `nevado-sherpa-tui/packages/`.
...
#### BQ3 — ~~Can the three terminal tools be injected by the runtime?~~ **RESOLVED: no — and target 4.0.2, not HEAD.**

Two parts, both settled.

**(a) There is no injection seam.** Evidence in §0.1.b C26. `getToolConfig` is not imported in `apps/runtime`; `buildToolConfig` is subtractive only; `AgentEngineOptions` and `AgentRunnerDeps` have no tools field; dispatch is a hardcoded `switch` whose default arm calls `executePlatformTool`, which is itself **a second closed `switch`** — so an unrecognised tool name has no handler at all. MCP cannot stand in: its tools are reached through the `listMcpTools`/`useMcpTool` meta-tools and never appear to the model as named tools with their own schemas.

So the release choreography for any change to a terminal tool's schema is: edit `sherpa-sdk` → tag and publish `@nevadoai/sherpa-core` to `npm.pkg.github.com` → bump `apps/runtime/package.json` and `pnpm-lock.yaml` → build → deploy by manual SSM RunCommand, which stops the service, `rm -rf`s `/opt/sherpa`, and loses every in-flight session. There is no shortcut: root `package.json`'s `pnpm.overrides` covers only `brace-expansion` and `esbuild`, and CI runs `pnpm install --frozen-lockfile`, so a local link will not survive.

**That recurring bill is the reason §A.5 now has two tools rather than three** — see §A.5.2 and §0.1.d(1).

**(b) Target version: patch 4.0.1 and publish 4.0.2. Do NOT bump to HEAD.** The earlier draft stated a preference for HEAD and then argued against it in the same paragraph; that was not a decision. It is one now, and the evidence is decisive:

- `sherpa-sdk` HEAD is **64 commits** and ~13 300 insertions ahead of `v4.0.1`, including relocating the engine under `engine/` and extracting `ToolDispatcher`/`ToolContext`.
- **`tool-specs.ts` is byte-identical between the two versions.** So the tool *spec* surface — the part carrying the schema risk — is fully portable. Only the dispatch and handler plumbing differs, and that work is comparable either way: at HEAD handlers read `ToolContext`, in 4.0.1 they reach `this.options` directly.
- The protocol package differs between the versions in exactly one file, purely additively (`session.ts` gains a `containerHost` doc comment and `worktreePath?`/`worktreeBranch?`). **Every type §A.8 touches is byte-identical.**
- The runtime has no deploy pipeline, no staging instance and no session drain. Bundling 64 commits with the first-ever machine consumer means that when the first cycle misbehaves you cannot tell whether the cause is your two tools or one of 64 commits.

Do the 64-commit bump as its own release, verified against human sessions, on the runtime owner's schedule — **ideally before Phase 2, never inside it.** Every `engine/tools/*` and `tool-context.ts` path cited in §0.1.c is a HEAD path; when you open the deployed 4.0.1 and the line number is wrong by hundreds, that is why (§0.1.b C29).
...
- The stated objection to changing the SDK — *"it loosens an invariant the SDK asserts deliberately"* — does not survive scrutiny. `this.userId` is the literal `'dev-user'` (`index.ts:61`) and nothing validates it, so the "invariant" is a placeholder in a service with no authentication, not a control.
...
1. **v1's ACL enforces `application === 'sdlc'` and nothing else.** That is the whole of its real security value: a machine caller cannot read a human transcript, and a human cannot resume an SDLC session. **It requires no SDK change and no per-request manager.** Per-client partitioning on `createdBy` is deferred until a second M2M client exists — with one client it partitions nothing, and §A.3 is already honest that the client certificate is the real boundary.
2. **Unforce `createdBy` and `application` in 4.0.2 anyway** — move both out of the forced block at `session-manager.ts:755-768` so an explicitly supplied `options` value wins, keeping the current values as defaults, and leaving `id`, `name`, `status`, `publishState`, `transcript` and `tombstone` forced. Two lines, riding a release that Task 2.1 is publishing regardless, so the marginal cost is zero. It buys two things: `createdBy: "m2m:<client_id>"` is populated from day one so the predicate can be added later without a migration, and it **removes a silent-override hazard** — today's `application` behaviour depends on the runtime's manager *not* being given one, so anyone who later passes `application: 'web'` to that constructor would silently start overriding `'sdlc'` and quietly disable the entire ACL.
3. **Assert the hazard in a test, because (2) may slip.** Task 2.2's test list includes an assertion that a created SDLC session actually persists `application === 'sdlc'`. That test is what catches it if someone adds an `application` to the manager's construction.

This deletes the old Task 2.0 as a blocking investigation (it is repurposed — see Phase 2) and reduces Task 2.4's ACL to a `preHandler` with one predicate.
...
| — | Decisions: `seq` → per-envelope UUID; SDK 4.0.2 not HEAD; `DELETE` returns `200`; the stray `201`/`202` contradiction. | BQ2, BQ3 above; §A.2.1, §A.2.4, §A.4.1, §A.4.5, Tasks 3.1/3.9, Phase 3 exit criteria. |
...
| 7 | **It rides the 4.0.2 release Task 2.1 already cuts.** | Marginal cost: **zero deploy windows.** Deferring it costs one. |
...
#### 0.9.1 Ingress: direct to the ALB. **Phase 1 collapses.**

**Four modes are implemented behind `var.sdlc_ingress_mode` — `none`, `token`, `mtls`, `header`. `token` is the one that runs; `mtls` and `header` are retained and inactive.** `none` is the default, deliberately, because every merge to `develop` triggers a full untargeted apply against testing's live ALB and nothing should enable an ingress merely because someone merged. `header` is the shared-secret variant described further below, kept as a fallback that needs no CA; its drawback is unchanged — an ALB `http_header` condition takes a literal, so the secret lives in config and state.

Everything below about deleting the API Gateway, the M2M client and the shared header secret *as the chosen design* holds for all of them — that part of this decision is unchanged.

##### `token` — the option being implemented

**The orchestrator calls `https://sherpa.<customer_subdomain>/v1/sdlc/*` on the existing 443 listener, presenting `Authorization: Bearer <token>`. The runtime verifies the token itself.**

- The token is read **once at boot** from Secrets Manager, so no request pays for a secret read and rotation needs a restart of both sides.
- Terraform **generates** it — `random_password` plus an `aws_secretsmanager_secret_version` — so there is no human step at all.
- The ALB rule sits at **priority 6** on the existing 443 listener and matches the path **and** an `Authorization: Bearer *` header. It cannot judge the token; the header condition reduces reachability rather than asserting a boundary. The runtime's own check is what authorises.
- An unconditional **priority-8 deny** sits below it, gated on the ALB alone rather than on the mode, so the namespace is never browser-reachable in any mode.
- `createdBy` is recorded as `m2m:bearer`. A shared secret carries no subject, so it identifies the credential and not the caller. A second machine client needs its own credential, not a parsed field.

**The trade, stated rather than implied:** a bearer token is replayable and a client certificate is not. Anyone who obtains the token can present it until it is rotated. Both are TLS-encrypted in transit, so the difference is what a copy is worth.

##### `mtls` — retained, not active

The 8443 listener, trust store and client-certificate design below remain implemented and selectable, so a deployment that already has a certificate is not forced to migrate.

**Why it is not the option we run — and the earlier reasoning here was wrong.** Earlier drafts treated mTLS as requiring an unexecutable manual step. That was a property of **our own Terraform**, not of mTLS:

- `verify` mode does require CA certificates. `aws_lb_trust_store` takes exactly two required arguments, an S3 bucket and key holding a CA bundle. That much is true.
- But a CA needs **no human step**. AWS Private CA self-signs a root entirely in Terraform: `aws_acmpca_certificate_authority` with `type = "ROOT"` exposes a CSR, `aws_acmpca_certificate` signs it with the `RootCACertificate/V1` template, `aws_acmpca_certificate_authority_certificate` activates it, and the CA certificate is then a plain attribute an `aws_s3_object` can write to the bucket the trust store reads. The `tls` provider can act as the CA for nothing.
- Both were **declined on cost**, not on feasibility: AWS Private CA is $400 per CA per month general-purpose, or $50 in short-lived mode with a 7-day certificate cap and therefore weekly reissue automation. The `tls`-provider route is free but puts the CA private key in Terraform state.
- The manual step in the original configuration was a **choice**: we declined to write a `secret_version` for the client key because it would land in state. The `token` mode then writes a generated credential into state anyway, so that rule was applied inconsistently. Both options put a credential in state; only one of them needed a person.

**Do not reintroduce the claim that mTLS was infeasible.** It was more expensive and we chose not to pay.

**Why not the API Gateway front door.** Option A put **two doors in series, either of which alone sufficed.** A caller needed both a `client_credentials` token *and* the ALB's condition; but the ALB is internet-facing, so **anyone satisfying the ALB condition bypassed the gateway entirely.** A door that can be walked around adds cost and moving parts, not protection.

**Why not a shared header secret.** It was the original ALB condition and it is now gone too. It is a static string that **must appear as a literal in Terraform config and state** — neither an `aws_lb_listener_rule` `http_header` condition nor any parameter mapping can resolve a secret at request time — so "rotate it" means editing two places and re-applying, and state-file access equals holding the credential. A client certificate has none of those properties.
...
| **BQ1** and architecture doc **U1** | **Moot.** No token, no authorizer in the path. It was answered YES on 2026-09-24 and §0.4 keeps the evidence as a platform fact for whoever next needs M2M auth |
| The shared secret, `var.sdlc_shared_secret`, and the `http_header` rule condition | Replaced by mTLS |
| `SDLC_M2M_CLIENT_SECRET_ARN`, `COGNITO_TOKEN_ENDPOINT`, `SDLC_M2M_SCOPE` | No token to fetch |
| The M2M token cache in `internal/runtimeclient` | **The client gets materially simpler**: load a cert and key, make one HTTPS call. No OAuth round trip, no `expires_in`, no 50-minute refresh |

##### The blocker, and the shape that resolves it

**ALB mutual authentication is a listener-level setting, and there is exactly one HTTPS listener.** Verified: `aws_lb_listener.sherpa_https` at `sherpa-ec2.tf:542` is the only one, serving both the human workspace UI (rules 10 and 20 on `/v1/workspace/*`, under `authenticate-cognito`) and everything else via a default action of `authenticate-cognito` + `forward`. **Putting `mode = "verify"` on it would demand a client certificate from every browser** and break the human surface outright.

**Resolution: a second HTTPS listener on port 8443, mTLS-only, forwarding to the same target group.**

```
                          ┌─ :443  authenticate-cognito  → /v1/workspace/*  → tg-sherpa   (humans, unchanged)
sherpa.<subdomain>  ──────┤        + rule 6: /v1/sdlc/* → fixed_response 403
                          └─ :8443 mutual_authentication{verify, trust_store}
                                   + rule 5: /v1/sdlc/* → tg-sherpa
                                   + default: fixed_response 403          (machines only)
```

Properties this buys, each verified rather than assumed:

- **Same hostname, same ACM certificate, same target group.** Only the port and the listener differ, so `aws_lb_target_group.sherpa` (`:505`) is untouched and there is **no second path to the instance**.
- **The instance security group stays exactly as it is.** Verified live: `sg-0d968534a4a9814dd` permits port 3000 **only** from `sg-0cb49292df9f5504e` (the ALB's SG) with **no CIDR ranges at all**. Because the new listener reuses the same ALB and the same target group, that remains the complete story — **the ALB is still the only route to a runtime that has no authentication of its own** (§0.1.b C18: `req.userId` is the literal `'dev-user'` and no route checks it).
- **The human path is untouched.** No edit to the 443 listener's default action, no edit to rules 10 or 20.
- **Separation is structural, at the listener**, rather than a header condition on a shared listener. That is a stronger boundary than anything the previous design had.

##### Why `verify` and not `passthrough` — record this so it is not "simplified" later

`mutual_authentication { mode = "passthrough" }` **does not require a certificate and does not validate one.** It forwards whatever the client presents (or nothing) in an `X-Amzn-Mtls-Clientcert` header and leaves all judgement to the application. So under passthrough:

- A client with **no certificate** still reaches the target group.
- A **self-signed** certificate with any subject the app happens to check would satisfy a header-condition or app-side string comparison, because nothing verified a chain.

That is not a gate, it is a transport convenience, and adopting it would quietly return this design to "a string the app compares" — the exact property that disqualified the shared secret. **Use `verify`, which rejects at the ALB any client whose certificate does not chain to the trust store, before the request reaches the instance.** Leave `ignore_client_certificate_expiry` at its default `false` so an expired certificate is refused rather than accepted.

##### The CA: a self-managed bundle in S3, not ACM Private CA

**Decision: self-managed CA, bundle in S3, private key in Secrets Manager.**

| | ACM Private CA | Self-managed bundle |
|---|---|---|
| Cost | **~$400/month** for the authority in general-purpose mode (~$4,800/year), plus per-certificate issuance. Short-lived-certificate mode is ~$50/month (~$600/year) | **Free.** S3 storage for one PEM; ALB trust stores carry no charge |
| What it buys | Managed CRL/OCSP, IAM-gated issuance, an audit trail of every certificate, ACM-integrated renewal | Nothing managed |
| What it costs you | Nothing operationally | **You own the key material, issuance, expiry tracking and revocation** |

**$4,800/year — or even $600 — to run a certificate authority whose entire population is one client certificate for one Lambda is not defensible.** ACM Private CA earns its price when you issue many certificates to many principals and need revocation and audit as managed capabilities. This issues one.

**What you own, stated plainly rather than waved through:**

- **Generate the CA offline** (`openssl req -x509 -newkey rsa:4096 -days 3650 …`). The CA **private key never goes near the ALB or the Lambda** — it is needed only to sign a leaf. Store it in Secrets Manager with a restrictive policy, or keep it offline entirely and accept that re-issuance needs a human.
- **The CA certificate** (public) goes to S3 as the trust-store bundle.
- **The leaf certificate and its key** go in one Secrets Manager secret, read by `internal/runtimeclient` at cold start and cached at module scope.
- **Expiry is yours to track.** A forgotten leaf expiry breaks every cycle at once, and the symptom is a TLS handshake failure that looks nothing like an application error. **Set a CloudWatch alarm or a calendar reminder at leaf-expiry-minus-30-days.** This is the real cost of the decision and it should be assigned to a person, not to the spec.

##### Rotation without a disagreement window — rotate the leaf, not the CA

**The trust store holds only the CA. So a new leaf signed by the same CA is accepted immediately, with no ALB change at all.** That gives zero-window rotation by construction:

1. Issue a new leaf from the existing CA.
2. Update the Secrets Manager secret.
3. Lambda picks it up on its next cold start (or sooner if the cache has a TTL — give it one).

**No Terraform apply, no trust-store update, no moment where one side has rotated and the other has not.** This is the property the shared secret could never have, and it is the strongest practical argument for mTLS here.

**CA rotation** — rare, on the order of years — uses the trust store's support for **multiple CA certificates**: add the new CA to the bundle so both old and new leaves validate, migrate the leaf, then remove the old CA. Also windowless, just slower.

##### What still needs deciding at implementation time

**The 8443 source CIDR has to be `0.0.0.0/0`, and that is not a choice.** Narrowing it would be free defence if it were possible, but **a non-VPC Lambda has no fixed egress IP** — its traffic leaves from the AWS-managed pool, and there is no documented per-region CIDR for Lambda egress that could be pinned. Anything narrower would break the caller. **So say it plainly in the Terraform comment: the security group is open to the internet on 8443 and mTLS is the control.** That is a genuine improvement over the previous design, where an open listener was gated by a string in state.
...
##### Caller identity under mTLS is real, and that is a change worth using

The previous design had a self-asserted `X-Nevado-Client-Id` — a label, not a credential. **mTLS gives the runtime a verified identity for the first time**: the ALB injects `X-Amzn-Mtls-Clientcert-Subject`, `-Issuer`, `-Serial-Number`, `-Validity` and `-Leaf`, and **those headers cannot be forged by a client because the ALB overwrites them.** §A.3 specifies how the runtime uses them, including the one trap: a request arriving on **443** carries no such header, so the runtime must **require the header's presence**, not merely check its value.
...
|---|---|---|
| `SessionStatus`, `SessionCommand`, `SessionApplication`, `TokenUsage`, `AgentSession`, `SESSION_NAME_MAX_LENGTH` | `sherpa-sdk/packages/protocol/src/session.ts` | exists; three additive changes needed (§A.8) |
| `CreateSessionRequest`, `CreateSessionResponse`, `GetSessionResponse`, `HealthResponse` | `sherpa-sdk/packages/protocol/src/api.ts` | exists; `CreateSessionRequest`/`CreateSessionResponse` extended (§A.8) |
| `ServerMessage` union | `sherpa-sdk/packages/protocol/src/messages.ts` | exists; one additive variant + one additive field (§A.8) |
| `PauseReason` | `sherpa-sdk/packages/core/src/engine/engine-types.ts:54-63` | exists, complete; needs re-exporting from `protocol` to reach the wire (§A.8) |
| `SdlcEventEnvelope`, `SdlcEvent`, `CompletionPayload`, `SdlcSessionProfile`, `SdlcCorrelation` | **new**, `sherpa-sdk/packages/protocol/src/sdlc.ts` | does not exist |
| CC-side mirror of the above | **new**, `backend/common/sdlcContract.js` | does not exist |

**Why a new `sdlc.ts` file rather than extending `session.ts`:** it keeps the one SDLC-shaped module in the protocol package isolated and greppable, so the boundary claim in architecture doc §1.1 ("the runtime never learns an SDLC concept") stays auditable — the runtime imports *types* from `sdlc.ts` and never reads a `correlation` field's contents. If these types were scattered through `session.ts` that property would be unverifiable.
...
**Option C** — see §0.9.1 for why, which supersedes architecture doc §2.1's Option A recommendation. The path from a CC Lambda to the runtime is one hop:

```
sdlc-cycle-worker (Go Lambda, no VPC config — zero vpc_config blocks exist in infrastructure/*.tf)
  1. read the client cert + key from Secrets Manager (cached, module scope)
  2. POST https://sherpa.<customer_subdomain>:8443/v1/sdlc/sessions
       (mTLS handshake presents the leaf; NO auth header of any kind)
     -> :8443 listener, mutual_authentication{mode=verify, trust_store}
          ALB rejects any leaf that does not chain to the CA, before the instance sees it
     -> rule priority 5: path_pattern /v1/sdlc/* -> forward to tg-sherpa
     -> default action: fixed_response 403
     -> ALB injects X-Amzn-Mtls-Clientcert-Subject / -Issuer / -Serial-Number / -Validity / -Leaf
        (client-supplied values are overwritten, so they cannot be forged)
     -> target group :3000 -> apps/runtime
```

**No API Gateway, no `HTTP_PROXY` integration, no Cognito M2M client, no OAuth round trip, no VPC link, no VPC attachment.**

Three consequences worth stating, because they change earlier text in this document:

- **Caller identity is verified for the first time.** The ALB sets `X-Amzn-Mtls-Clientcert-*` from the presented leaf and **overwrites anything the client sent**, so those headers cannot be forged. That replaces the self-asserted `X-Nevado-Client-Id` of earlier drafts, which was a label rather than a credential. §A.3 specifies how the runtime uses them — including the trap that a request arriving on **:443** carries no such header, so **presence must be required, not merely the value checked**.
- **The client is simpler.** `internal/runtimeclient` (Task 2.6) reads one secret (the cert/key pair), builds a `tls.Config` with it, and makes one HTTPS call. No token endpoint, no `expires_in`, no 50-minute refresh. Go's `crypto/tls` does client certs natively via `tls.X509KeyPair` — no dependency.
- **`sherpa.<customer_subdomain>:8443` must be resolvable and reachable from the Lambda.** The name is public (`frontend.tf:243-255` is the only existing consumer) and a non-VPC Lambda has default internet egress, so this should hold — **but verify it before Task 2.6 on both counts**: nothing has ever called this host from a Lambda, and **8443 is a non-standard port that has never carried traffic.** A DNS record pointing at the ALB serves every port the ALB listens on, so no DNS change is needed.

Two notes that are contract, not commentary:Two notes that are contract, not commentary:

- **Rule 6 is mandatory.** Verified live: the ALB's default action is `authenticate-cognito` with `OnUnauthenticatedRequest: "authenticate"` (i.e. a 302 to the hosted UI) followed by `forward`. Existing rules are priority 10 (`/v1/workspace/auth`, actions `redirect` + `authenticate-cognito`) and 20 (`/v1/workspace/*`, actions `authenticate-cognito` + `forward`). A `/v1/sdlc/*` request with a wrong or missing header falls past both to the default and receives **an HTML login redirect, not a 403**. Rule 6 turns that into a 403.
- **`overwrite:` is mandatory on both mapped headers.** Without it a caller supplies their own and it survives. This is the whole of the ACL's integrity (§A.3).

`AccessTokenValidity` on a credmanager-minted client is **60 minutes** (`create.go:72-73`). Cache the token with a safety margin; do not mint one per request.
...
```ts
// sherpa-sdk/packages/protocol/src/sdlc.ts
...
`CreateSessionResponse` today is `{sessionId?: string, error?: string}` (`api.ts:15-18`). Three added fields, all optional in the type so the human path keeps type-checking (§A.8 P7).
...
| 403 | Caller holds neither `commandcenter/cycles.write` nor `commandcenter/full_access`; or the ALB shared-secret header is wrong/missing (returned by ALB rule 6) |
...
**⚠ Prerequisite: Task 1.2b.** Command Center's own machine-token classifiers (`middleware/auth.go:20-23`, `tenantHelpers.js:11-14`) both require `sub` to be absent, and a real `client_credentials` token **carries `sub`, equal to `client_id`** — verified live on 2026-09-24 (§0.4 BQ1). So they return `false` for every machine token that exists. Nothing in this section can be built on them until that is fixed. **The runtime-side predicate below is unaffected** (it is `application === 'sdlc'` on the session record, not a claim test), but the CC side of the ingress — scope checks, audit attribution — is.
...
**Per-client partitioning on `createdBy` is deliberately deferred.** With one M2M client it partitions nothing, and §A.3's own posture statement is that the mTLS client certificate is the real boundary. `createdBy: "m2m:<client_id>"` is still **written** from day one — free once 4.0.2 unforces it (BQ4 part 2) — so the predicate can be added later with no migration and no backfill. **State this as a v1 limitation in the code comment and the runbook, not as a design.** When a second M2M client appears, add the predicate; until then, claiming multi-client isolation would be false.

**What caller identity is worth, and it is now worth something.** Earlier drafts had no verified identity: a header the caller set itself, on a listener gated by a static string. **Under mTLS (§0.9.1) the ALB authenticates the client certificate and injects the result**, overwriting anything the client sent, so `X-Amzn-Mtls-Clientcert-*` **cannot be forged.**

**Still do not call the ACL an authorization system** in any doc, commit message or runbook. It is a machine/human separation plus a verified caller label. Architecture doc R2 exists because someone will otherwise cite it as evidence of isolation.

Headers the runtime reads, both set with `overwrite:` by the API Gateway integration:

| Header | Value | Runtime use |
|---|---|---|
| `X-Amzn-Mtls-Clientcert-Subject` | set by the ALB from the verified leaf | **Required.** Its absence means the request did not arrive through the :8443 mTLS listener — reject with `403`. The CN (`sdlc-cycle-worker`) becomes `createdBy: "m2m:<CN>"` |
| `X-Amzn-Mtls-Clientcert-Serial-Number` | set by the ALB | log it. It is what identifies *which* leaf called, and therefore what you revoke |
| `X-Amzn-Mtls-Clientcert-Issuer`, `-Validity`, `-Leaf` | set by the ALB | available; not required by v1 |

**Requiring presence, not just checking the value, is the load-bearing detail.** The :8443 and :443 listeners share one target group, so the runtime cannot tell from the request alone which door it came through — **except** that only :8443 produces these headers. A handler that reads the subject when present and shrugs when absent is reachable from :443 by any authenticated human (which rule 6 blocks at the ALB, but defence in depth here costs one line). **Treat a missing `-Subject` as a hard `403`.**

**Subject verification is recommended for v1, and it is cheap.** `verify` mode accepts *any* leaf the CA signed. Today the CA signs exactly one, so the two are nearly equivalent — but the moment it signs a second (a developer's cert, a second service) an unchecked subject means they are interchangeable. **Check the CN against an allowlist from config**, ~10 lines, and it makes the CA reusable without widening access.

**Use the `m2m:<client_id>` format so audit records correlate with CC's.** Command Center *intends* the same string — `backend/common/tenantHelpers.js:29-31` and `middleware/auth.go:33-35` both build `` `m2m:${client_id}` `` — but **neither branch is reachable today**: both check `sub` first and a `client_credentials` token carries `sub`, so they return the bare client id (Task 1.2b). The runtime composes the string from the `X-Nevado-Client-Id` header directly and is unaffected; the point of matching the format is that CC's side will produce it too once 1.2b lands.

Rules, enforced at the runtime on the session record. **The right seam is a Fastify `preHandler` hook inside the `/v1/sdlc` encapsulation** — a plugin registered with a prefix is its own scope, so `instance.addHook('preHandler', …)` applies to `/v1/sdlc/*` only and **cannot leak onto the human `/v1/workspace` routes**. Do not add it to the root instance.

| Operation | Rule |
|---|---|
| **Create** | Caller must hold `commandcenter/cycles.write` or `commandcenter/full_access`. Both exist live on resource server `commandcenter`. **Enforce the scope at API Gateway** — route-level `authorization_scopes`, Task 1.3 — not in application code, for two reasons: API Gateway is the layer holding the verified token, and **application-side scope enforcement does not work today.** `HasScope` (`auth.go:138-152`) treats `full_access` as a wildcard at `:147`, which is the semantics to mirror if it is ever wired up — but it has **zero callers**, and its `IsMachineToken` guard at `:139` returns `false` for every real machine token (Task 1.2b), so it would refuse every M2M caller even if called. **Do not build the ACL on it.** The session is stamped `application: "sdlc"` and `createdBy: "m2m:<client_id>"`. |
| **Read (`GET`)** | **Only sessions with `application === 'sdlc'`.** Anything else — a human `web`/`ide`/`tui` session, or a session with no `application` — returns `404`, **not `403`**: a `403` makes the endpoint a session-id oracle. Under the SQS design this `GET` is the *only* read path a machine caller has, which makes it both narrower and more important than it looks. |
| **Cancel / Delete** | Same predicate, same `404` on anything else. |
| **Per-client partitioning** | **Deferred to v2.** `createdBy` is written but not read. See above. |
| **Settings** | **`PUT /v1/workspace/settings` must not be reachable from the SDLC ingress.** It mutates the process-wide settings store and writes `~/.config/nevado/settings.json` (`routes/settings.ts:24-28`), so a machine caller could flip `autoApproveAllCommands` for **every session on the box, human included** (§0.1.b C28). The :8443 listener's rule 5 scopes it to `/v1/sdlc/*` and its default action 403s everything else, which is what keeps this out of reach — so the requirement is "do not add a settings route under `/v1/sdlc`", and it is worth an explicit test that no such route exists. |
...
**`createdBy` is a repurposing that needs a comment.** `session.ts:117-118` documents it as human identity — `/** User identity — e.g. "jeff", "tyler" */`. The `m2m:` prefix is what distinguishes the two namespaces; nothing enforces it. §A.8 adds the comment.
...
export interface SdlcEventEnvelope {
  ...
  readonly seq?: number;               // OPTIONAL, non-durable, log readability only.
                                       //   In-memory per-session counter. NOT the dedup key.
                                       //   Resets on restart, and that is fine.
...
- **`seq` is optional, in-memory, and explicitly not the dedup key.** It exists so a human reading logs can see gaps. It resets on restart; nothing depends on it; **do not persist it.** An earlier draft made it a durable counter, which had a silent catastrophic failure mode (a reused id is dropped by SQS with no error) and put a process-globally-locked whole-file rewrite of `sessions.json` on the publish hot path. BQ2 has the full reasoning. If keeping `seq` at all invites someone to depend on it, drop it — nothing in this spec reads it.
...
- Both drifts are **identical at HEAD**. `tool-specs.test.ts` tests only name filtering. **There is no spec↔handler conformance test anywhere.**
...
**But note the enforcement caveat from §A.5.0a applies here too:** the declared schema is not enforced by anything. So the handler must hand-check every field it uses, and — per the drift finding — **a test must bind the declared spec to the handler.** Task 2.1 adds one.

Behaviour, normative:

| Case | Behaviour |
|---|---|
| `outcome: 'blocked'` | No commit, no push. `pushed: false`; `commitSha`/`branch` from current HEAD. `blockedReason` REQUIRED — absent is a `toolError`. |
...
| Push failure (non-fast-forward, auth) | `toolError` carrying git's **stderr**; the agent can rebase and retry. `ssmBuildRunner.gitPushWithRetry` (`:438-459`) retries 3× — preserve that, but the final failure **must reach the agent**, not be swallowed. |
| Arguments arrive as `{raw: "…"}` | `toolError`. This is the laundered-JSON case (§A.5.0a) and it must not be mistaken for a blocked outcome. |
| Success | The returned `ToolResult` content **is** the `EngineeringCompletion` payload, so the agent sees what was recorded. |

**This is new code, not wiring.** `sherpa-sdk/packages/core` has **no commit or push primitive at all** — verified by an exhaustive git-subcommand census: `rev-parse`×12, `worktree`×6, `status`×6, `fetch`×3, `write-tree`×2, `stash`×2, `reset`×2, `remote`×2, `add`×2, plus single uses of `symbolic-ref`, `merge`, `ls-remote`, `log`, `commit-tree`, `cat-file`, `branch`. There is **no porcelain `git commit`** — the only commit-creating code is plumbing `commit-tree` in `git-isolate.ts:228-244`, building dangling commits to snapshot dirty state and advancing no branch. There is **no `git push`** — the only `'push'` string in the package is `git stash push -u -m` at `resume-guard.ts:717`.

So `reportCompletion` must implement, from nothing: stage-all, commit with the agent's message, push with retry, **empty-tree detection**, branch resolution, and useful stderr surfacing. It also needs the model id and the session's cumulative `TokenUsage` threaded to the handler, which the deployed 4.0.1 reaches via `this.options`.
...
1. **The seam can ship before it is used.** The 4.0.2 release carries the validator; Command Center turns it on later by starting to send `resultSchema`. No coordination.
...
**It rides the 4.0.2 release, so its marginal cost is zero deploy windows.** Task 2.1 already cuts that release for `reportCompletion`, `submitPlan`, `submitReview`, `PLAN_BLOCKED_TOOLS`, `TOOL_METADATA`, the `buildToolConfig` allowlist filter and the `SessionManager` unforcing (§A.8 P10–P12). **Deferring `resultSchema` to "later if malformed payloads prove common" costs one outage window; including it now costs none.** When the release mechanism is the binding constraint, the seams go in on the release you are already taking.
...
`buildToolConfig(mode)` filters `getToolConfig({extended: true, mode})` by provider availability, then removes `PLAN_BLOCKED_TOOLS` in plan mode. `PLAN_BLOCKED_TOOLS` is a `Set` at `tool-specs.ts:522` — `new Set(['editFile', 'writeFile'])` — independently enforced inside `getToolConfig` at `:532`. `buildToolConfig` is **purely subtractive**: `rawConfig.tools.filter(...)`, no concat anywhere.

| Change | Why |
|---|---|
| Add `reportCompletion` to the `do` set | the engineering terminal tool |
| Add `reportCompletion` to `PLAN_BLOCKED_TOOLS` | a plan session must not commit |
| Add `submitPlan` and `submitReview` to the `do` **and** `plan` sets; block neither | `submitPlan` is used by the `plan`-mode planning session and `submitReview` by the `do`-mode QA session. Mode gating cannot express "plan-mode only", so **the per-role allowlist is what separates them** (§A.7.3) — and it does so exactly, since each role's list names one terminal tool |
| Add `submitReview` to `PLAN_BLOCKED_TOOLS`? **No** | it would be harmless but it is not mode that distinguishes these; putting half the separation in mode gating and half in the allowlist makes both harder to reason about. **One mechanism: the allowlist.** |

**The allowlist is the real control; mode gating is the backstop for it.**
...
**Each role gets exactly one terminal tool, and the allowlist is what guarantees it.** `plan` → `submitPlan`, `engineer` → `reportCompletion`, `qa` → `submitReview`. Mode gating cannot express this (both `submitPlan` and `submitReview` must be available in some mode), so the allowlist is the sole separator — **assert the full sorted set in a test, not `toContain`**, because the whole point of an allowlist is that nothing arrives by default.
...
- **`listMcpTools`, `useMcpTool`** — provider-gated, so they disappear when nothing is configured (`agent-engine.ts:419-422`). But an SDLC session must not inherit whatever MCP servers were wired for the human surface.
...
### A.8 Protocol changes required in `@nevadoai/sherpa-protocol` and `-core`

All additive. **None may be breaking**, because the runtime and CC cannot be released atomically (§0.3). **Target version: 4.0.2, patched from 4.0.1 — not HEAD** (BQ3(b)).
...
**Ordering.** P1–P12 land in **one** 4.0.2 release. Then `nevado-sherpa-tui` bumps `apps/runtime/package.json` + `pnpm-lock.yaml`; then `command-center/frontend` bumps and `backend/__tests__/sherpaSdkDriftGuard.test.js:40`'s `PINNED_SDK_VERSION` is updated in the same PR — **that test fails until it is, which is the point.** The 64-commit HEAD bump is a separate release on the runtime owner's schedule (BQ3(b)).
...
**This phase is empirical.** The runtime and its ALB are live in testing (verified: ALB `testing-sherpa-alb`, `active`, internet-facing; instance `i-0d5271e1ce5ea4fb0` named `testing-sherpa-runtime` at `10.16.2.183` in `vpc-02482e2c1d40e003b`). So the auth question is a curl, not a design argument.

**Ordering principle: establish the credential before anything depends on it.** Tasks run 1.4 Step 1 (CA + leaf, by hand) → 1.4 Steps 2–3 (Terraform) → 1.5 → 1.7, with 1.6 in parallel. **Phase 1 is now four tasks**, not seven — §0.9.1 deleted the API Gateway front door and with it Tasks 1.1, 1.2, 1.2b and 1.3.
...
Constants you will need, all verified live:

| | |
|---|---|
| ALB | `testing-sherpa-alb-1426385821.us-east-1.elb.amazonaws.com`. **One** HTTPS listener (`aws_lb_listener.sherpa_https`, `sherpa-ec2.tf:542`) on **443**, default action `authenticate-cognito` (`OnUnauthenticatedRequest: authenticate`) then `forward` to `testing-sherpa-tg`. **Task 1.4 adds a second listener on 8443** |
| ALB rules in use | priority **10** (`/v1/workspace/auth`, `redirect` + `authenticate-cognito`), priority **20** (`/v1/workspace/*`, `authenticate-cognito` + `forward`). Priorities **5 and 6 are free** on :443 |
| Target group | `aws_lb_target_group.sherpa` (`sherpa-ec2.tf:505`). **Both listeners share it** — that is what keeps the instance SG unchanged |
| ALB security group | `sg-0cb49292df9f5504e` — ingress **443 only**, from `0.0.0.0/0`. Task 1.4 adds 8443 |
| Instance security group | `sg-0d968534a4a9814dd` — port **3000 only**, sourced **only** from `sg-0cb49292df9f5504e`, **no CIDRs**. Verified live. The ALB is the sole route to a runtime with no authentication of its own |
| Runtime port | 3000; `GET /health` is at the **root**, not under a prefix |
| Hostname / certificate | `sherpa.<customer_subdomain>` with `var.acm_certificate_arn`. **Unchanged by this phase** — the new listener reuses both, so no DNS or certificate work |
...
**What replaces the ingress work: Tasks 1.4 (the two ALB listener rules), 1.5 (deploy targets) and 1.6 (the `/v1/sdlc` namespace).** That is the whole of Phase 1 now.

**The mTLS client certificate becomes the entire boundary, and Task 1.4 is therefore the only control.** Previously there were two doors and either sufficed alone; §0.9.1 explains why that bought nothing. Task 1.4's :443 catch-all (priority 6) is load-bearing for a **different** reason than earlier drafts gave: the :443 default action is `authenticate-cognito` **plus `forward`**, so without it an already-authenticated human browser reaches the machine namespace — a successful request, not a redirect.

### Task 1.4 — the mTLS listener, the trust store, and the 443 catch-all

**Files:**
- `infrastructure/sherpa-ec2.tf` (MOD — trust store, second listener, two rules, one SG rule)
...
#### Step 1 — the CA, by hand, once

**Before any Terraform.** §0.9.1 decided a self-managed CA over ACM Private CA.

```
# CA — 10 years. The private key NEVER leaves this step.
openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 -nodes \
  -keyout sdlc-ca.key -out sdlc-ca.crt \
  -subj "/CN=Nevado SDLC Runtime CA/O=Nevado AI"

# Leaf for the orchestrator — 1 year.
openssl req -newkey rsa:2048 -nodes -keyout sdlc-client.key -out sdlc-client.csr \
  -subj "/CN=sdlc-cycle-worker/O=Nevado AI"
openssl x509 -req -in sdlc-client.csr -CA sdlc-ca.crt -CAkey sdlc-ca.key \
  -CAcreateserial -days 365 -sha256 -out sdlc-client.crt
```

| Material | Destination |
|---|---|
| `sdlc-ca.crt` (public) | **S3**, as the trust-store bundle. Use a prefix in an existing bucket rather than creating one: `s3://<sessions-bucket>/sdlc-mtls/sdlc-ca.crt`. **Verify ELB can read it at apply time** — the trust store fetches the object when created or updated, and a restrictive bucket policy fails this with an unhelpful error |
| `sdlc-ca.key` | **Offline, or Secrets Manager under a policy no Lambda role can reach.** Needed only to sign a new leaf. It must not be readable by the runtime or the orchestrator |
| `sdlc-client.crt` + `sdlc-client.key` | **One** Secrets Manager secret as a JSON pair — `command-center/sdlc-client-cert`, `{"cert": "-----BEGIN…", "key": "-----BEGIN…"}`. Task 2.6's client reads this one secret |

**Record the leaf's `notAfter` and set an alarm at minus 30 days.** §0.9.1 explains why this is the real cost of the decision: a forgotten expiry fails every cycle at once, and a TLS handshake failure looks nothing like an application fault. **Assign it to a person, not to the spec.**

#### Step 2 — Terraform

```hcl
# Only the CA goes in the trust store. That is what makes leaf rotation need no
# ALB change at all — see §0.9.1.
resource "aws_lb_trust_store" "sherpa_sdlc" {
  count    = local.enable_sherpa_alb ? 1 : 0
  provider = aws.sherpa_runtime

  name                             = "${var.customer_id}-sherpa-sdlc-ca"
  ca_certificates_bundle_s3_bucket = aws_s3_bucket.sessions.bucket
  ca_certificates_bundle_s3_key    = "sdlc-mtls/sdlc-ca.crt"
}

# A SECOND listener, because mutual authentication is listener-level and :443
# serves human browsers under authenticate-cognito — verify mode there would
# demand a client certificate from every browser. Same hostname, same ACM cert,
# same target group; only the port and the auth differ.
resource "aws_lb_listener" "sherpa_sdlc_mtls" {
  count    = local.enable_sherpa_alb ? 1 : 0
  provider = aws.sherpa_runtime

  load_balancer_arn = aws_lb.sherpa[0].arn
  port              = 8443
  protocol          = "HTTPS"
  ssl_policy        = <copy from the :443 listener — do not invent one>
  certificate_arn   = var.acm_certificate_arn

  mutual_authentication {
    mode            = "verify"
    trust_store_arn = aws_lb_trust_store.sherpa_sdlc[0].arn
    # Default false. Explicit because an expired leaf must be refused.
    ignore_client_certificate_expiry = false
  }

  # Nothing but /v1/sdlc/* is reachable on this port.
  default_action {
    type = "fixed-response"
    fixed_response {
...
resource "aws_lb_listener_rule" "sherpa_sdlc_forward" {
  count        = local.enable_sherpa_alb ? 1 : 0
  provider     = aws.sherpa_runtime
  listener_arn = aws_lb_listener.sherpa_sdlc_mtls[0].arn
  priority     = 5

  condition { path_pattern { values = ["/v1/sdlc/*"] } }
...
# :8443 open to the world, and this is not a choice — see below.
resource "aws_security_group_rule" "sherpa_alb_mtls_ingress" {
  count    = local.enable_sherpa_alb ? 1 : 0
  provider = aws.sherpa_runtime

  security_group_id = aws_security_group.sherpa_alb[0].id
  type              = "ingress"
  from_port         = 8443
  to_port           = 8443
  protocol          = "tcp"
  cidr_blocks       = ["0.0.0.0/0"]
  description       = "SDLC mTLS listener. Open by necessity: the caller is a non-VPC Lambda with no fixed egress IP, so no narrower CIDR exists. mTLS is the control, not the SG."
}
```

Copy the `count`/`provider`/`listener_arn` idiom from the existing rules at `sherpa-ec2.tf:578-617` (priority 10) and `:619-650` (priority 20) — both gate on `local.enable_sherpa_alb` (`:416`) and use `provider = aws.sherpa_runtime`. **Check the real resource names and the :443 listener's `ssl_policy` before copying**, and reuse that policy rather than choosing one.

**On the open CIDR — put this in the Terraform comment, not only here.** A non-VPC Lambda's traffic leaves from the AWS-managed pool and there is **no documented per-region Lambda egress CIDR** to pin, so nothing narrower than `0.0.0.0/0` works. Narrowing would have been free defence if it were possible; it is not, and saying so beats specifying something unworkable. It remains a **strict improvement** on the previous design, where an equally open listener was gated by a static string that lived in Terraform state.

#### Step 3 — keep a 403 catch-all on :443, for a stronger reason than before

**Retain a `fixed_response` 403 for `/v1/sdlc/*` on the 443 listener.** Its original justification was cosmetic — avoid a confusing Cognito `302`. **The real reason is a security one.**

Verified live: the 443 listener's default action is `authenticate-cognito` **followed by `forward`**, and rules 10/20 match only `/v1/workspace/*`. So `https://sherpa.<subdomain>/v1/sdlc/health` on **443** falls through to the default and **an already-authenticated human browser is forwarded into the machine namespace.** Not a redirect — a successful request.
...
**Priorities 5 and 6 are both free** on :443 (verified live: only 10, 20 and the default exist). Rule 6 is on **:443**, rule 5 on **:8443** — different listeners, so the numbers do not interact, but keeping 5 for "forward the machine path" and 6 for "deny it elsewhere" reads consistently.

**The 8443 listener needs no catch-all rule**, because its **default action** is the 403 — strictly more robust than a rule, since a default cannot be mis-prioritised.

#### Acceptance

```
cd infrastructure && terraform plan \
  -target=aws_lb_trust_store.sherpa_sdlc \
  -target=aws_lb_listener.sherpa_sdlc_mtls \
  -target=aws_lb_listener_rule.sherpa_sdlc_forward \
  -target=aws_lb_listener_rule.sherpa_sdlc_deny_443 \
  -target=aws_security_group_rule.sherpa_alb_mtls_ingress
```
Expected: `Plan: 5 to add, 0 to change, 0 to destroy.` **Zero changes is the assertion that matters** — anything in the `change` column means the human path was touched.
...
```
Expected: `443` with `MTLS: "off"` — **unchanged, and this is the check that the human surface survived** — and `8443` with `MTLS: "verify"`.
...
```
Expected: **unchanged** — port 3000 reachable only from `sg-0cb49292df9f5504e`, no CIDRs. Reusing the same target group is what preserves it; re-assert it after the apply rather than trusting it.

Behavioural checks are Task 1.7's N4/N5/N6.

#### Rollback

These attach to a **live** ALB serving the human workspace UI, but every resource is **additive**: a listener on an unused port, a rule at an unused priority, an SG rule, a trust store. **Nothing existing is modified**, which is what makes this safe to apply during the day.

`terraform destroy` on the five targets, or `aws elbv2 delete-listener` in an emergency. **Apply order matters in one direction:** rule 6 before the 8443 listener means `/v1/sdlc/*` returns 403 everywhere — the safe direction. The reverse leaves a window where an authenticated human can reach `/v1/sdlc/*` on 443; harmless today because no SDLC route exists yet, but **apply them together**, and prefer rule 6 first if they must be split.

**Commit:** `feat: add an mTLS listener for the SDLC runtime namespace`
...
#### Under mTLS, this task adds **nothing** — and that is the finding, not an omission

**Every resource Task 1.4 creates is gated on `local.enable_sherpa_alb`, which is `false` on internal.** So none may be added to `deploy-dev.yml`'s target list: **a `-target` for a resource whose `count` is 0 fails the apply.**
...
|---|---|
| `aws_lb_trust_store.sherpa_sdlc` | **No** — `count = 0` on internal |
| `aws_lb_listener.sherpa_sdlc_mtls` | **No** — same |
| `aws_lb_listener_rule.sherpa_sdlc_forward` | **No** — same |
| `aws_lb_listener_rule.sherpa_sdlc_deny_443` | **No** — same |
| `aws_security_group_rule.sherpa_alb_mtls_ingress` | **No** — same |
| Task 1.7's IAM edit to `aws_iam_role_policy.lambda_policy` | **Already targeted** at `deploy-dev.yml:428` |

**So Phase 1 adds no target-list entries at all.** These resources reach testing through the **full untargeted apply** in `deploy-customer-instance.yml` (§0.3), which is the only workflow that deploys the account where the runtime exists.
...
### Task 1.6 — add the `/v1/sdlc` namespace and a health endpoint to the runtime

**Files (all in `nevado-sherpa-tui`):**
- `apps/runtime/src/config.ts` (MOD — add `SDLC_PREFIX`)
- `apps/runtime/src/routes/sdlc.ts` (NEW)
- `apps/runtime/src/routes/sdlc.test.ts` (NEW)
- `apps/runtime/src/index.ts` (MOD — register the plugin)
- `apps/runtime/src/config.test.ts` (MOD — if you add env vars; not needed for this task)
...
Create `apps/runtime/src/routes/sdlc.ts` with a single route for now:

```ts
export function registerSdlcRoutes(app: FastifyInstance, worker: AgentWorker, ptyManager: PtyManager, startTime: number): void {
  app.get<{ Reply: HealthResponse }>('/health', async () => ({
    status: 'ok',
    activeSessions: worker.activeCount,
    queuedSessions: worker.queuedCount,
    activePtySessions: ptyManager.activeCount,
...
await app.register(async function sdlcRoutes(instance) {
  registerSdlcRoutes(instance, worker, ptyManager, startTime);
}, { prefix: SDLC_PREFIX });
```
...
**Do not add the ACL hook yet.** That is Phase 2 Task 2.4. An unauthenticated health endpoint on the new namespace is exactly what Task 1.7's negatives need — N5 proves a valid client certificate reaches it, N6 proves an untrusted one does not.
...
**Acceptance:**
```
cd /Users/jwoolley/dev/nevadoai/nevado-sherpa-tui && pnpm build && pnpm --filter @nevado/runtime test
```
...
### Task 1.7 — wire the client certificate and run the three negatives

**Files:**
- `infrastructure/lambdas.tf` (MOD — env vars + IAM on the new Go worker)
- `.github/workflows/deploy-dev.yml` (MOD, if any target is missing — see Task 1.5)

**Two env vars, not five.** §0.9.1 removed the M2M plumbing:

| Name | Value |
|---|---|
| `SDLC_RUNTIME_API_BASE` | `"https://sherpa.${var.customer_subdomain}:8443/v1/sdlc"` — **the ALB's mTLS listener.** Note the **port**; omitting it silently targets :443, where rule 6 returns `403` and the symptom looks like a permissions problem |
| `SDLC_CLIENT_CERT_SECRET_ARN` | the Secrets Manager ARN of the `{cert, key}` pair from Task 1.4 Step 1 |

Gone: `COGNITO_TOKEN_ENDPOINT`, `SDLC_M2M_CLIENT_SECRET_ARN`, `SDLC_M2M_SCOPE`, `SDLC_SHARED_SECRET_ARN`.

**Why the base URL needs an env var at all:** the runtime's URL is exposed to nothing today. The only reference anywhere is the output at `sherpa-ec2.tf:709-712`, and the only consumer of `sherpa.<subdomain>` is the CloudFront origin at `frontend.tf:243-255`. `sherpaConfig` returns no URL field either.

**IAM:** `secretsmanager:GetSecretValue` scoped to that one secret ARN, on the Go worker's role. If the worker reuses `aws_iam_role.lambda_execution_role`, that policy is **already in the Core `-target=` list** at `deploy-dev.yml:428`, so editing it deploys with no new target entry.

#### The three negatives — the whole auth surface

An earlier revision had five. **N1 and N2 tested API Gateway and are deleted with it; N3 collapses into N5.** N6 is new and is the one that actually proves mTLS is doing something.

Generate a throwaway self-signed cert once, for N6:
```
openssl req -x509 -newkey rsa:2048 -nodes -days 1 \
  -keyout /tmp/rogue.key -out /tmp/rogue.crt -subj "/CN=sdlc-cycle-worker"
```
Note it deliberately carries **the same CN** as the real leaf. That is the point: it isolates "was the chain verified?" from "was the subject checked?".

**N4 — no client certificate on :8443 → refused at the TLS layer.**
```
curl -sS -o /dev/null -w '%{http_code}\n' https://sherpa.testing.nevado.ai:8443/v1/sdlc/health
```
Expected: **a TLS handshake failure, not an HTTP status.** `curl` exits non-zero (typically 35 or 56) and prints something like `alert certificate required`. **An HTTP `403` here would mean `mode` is `passthrough` rather than `verify`** — the request reached the listener and was refused by a rule instead of by the handshake. **A `200` means the SDLC namespace is open to the internet. Stop everything.**

**N4b — `/v1/sdlc/*` on :443 → `403`, not a Cognito `302` and not a `200`.**
```
curl -sS -o /tmp/n4b.txt -w '%{http_code}\n' --max-redirs 0 \
  https://sherpa.testing.nevado.ai/v1/sdlc/health
cat /tmp/n4b.txt
```
Expected: **`403`** with `{"error":"forbidden"}` — from rule 6.

- A **`302`** to `testing-control-center.auth.us-east-1.amazoncognito.com` means rule 6 is missing: the request fell through to the :443 default, which is `authenticate-cognito` with `OnUnauthenticatedRequest: authenticate` (verified live).
- A **`200`** would mean the default's `forward` was reached — i.e. **an authenticated human can call the machine namespace.** That is the hole rule 6 exists to close (Task 1.4 Step 3).

**N5 — valid client certificate on :8443 → `200`.**
```
curl -sS -w '\n%{http_code}\n' \
  --cert /tmp/sdlc-client.crt --key /tmp/sdlc-client.key \
  https://sherpa.testing.nevado.ai:8443/v1/sdlc/health
```
Expected: `200` with the runtime's `HealthResponse`. This is the **phase exit criterion**: the handshake verified against the trust store, rule 5 matched, the target group forwarded, and the runtime served the new namespace.

**N6 — a certificate the CA did not sign → refused, even with the right CN.**
```
curl -sS -o /dev/null -w '%{http_code}\n' \
  --cert /tmp/rogue.crt --key /tmp/rogue.key \
  https://sherpa.testing.nevado.ai:8443/v1/sdlc/health
```
Expected: **a TLS handshake failure** (`unknown ca` / `certificate verify failed`), exit non-zero. **A `200` means the trust store is not being enforced** — most likely `mode = "passthrough"`, or a trust store pointing at the wrong S3 object. **This is the test that distinguishes real mTLS from transport theatre**, and it is why §0.9.1 records the passthrough reasoning: without N6, a passthrough listener plus a subject check would pass N4b and N5 and look identical while gating on nothing.

**Also worth running once**, since it is the only check on the leaf-expiry alarm: confirm `ignore_client_certificate_expiry` is `false` by checking the listener, and record the leaf's `notAfter` in the PR.
```
openssl x509 -in /tmp/sdlc-client.crt -noout -enddate
```

#### The posture, recorded

**Write N5's and N6's results into the PR verbatim.** Together they state the boundary precisely: reaching the runtime's SDLC namespace requires a private key signed by a CA whose key never leaves an offline step — **not** possession of a string that lives in Terraform state. A session there still runs arbitrary commands with `bedrock:InvokeModel` on `*`, `secretsmanager:GetSecretValue` on the GitHub App private key, and push access to every repository the installation covers (§A.7.4), so the certificate is a real credential and should be treated as one: **rotation on a schedule someone owns (§0.9.1 — leaf only, no ALB change), an alarm at expiry minus 30 days, and revocation by re-issuing the CA bundle if the leaf is ever exposed.**

**Rollback:** the env vars are additive and unread until Phase 2. **N4 or N6 returning `200` is the only result here that is an incident**, and the rollback is `aws elbv2 delete-listener` on :8443, which closes the path immediately at the cost of breaking N5.

**Commit:** `feat: wire the SDLC runtime base URL and client certificate`

**Rollback:** the env vars are additive and unread until Phase 2. **N4 failing in the open direction is the only result here that is an incident**, and its rollback is `aws elbv2 delete-rule` on rule 5, which closes the path immediately at the cost of breaking N5.

**Commit:** `feat: wire the SDLC runtime base URL and shared secret`

### Phase 1 exit criteria

All six, in order. Nothing downstream starts until all six pass.

1. **The CA and leaf exist, and their handling is recorded.** The CA certificate is the trust-store bundle in S3; **the CA private key is offline or under a policy no Lambda role can reach**; the leaf's cert and key are one Secrets Manager secret; and the leaf's `notAfter` is in the PR with an alarm set at minus 30 days (Task 1.4 Step 1).
2. **`terraform plan` showed `0 to change`** on the Task 1.4 apply, and `describe-listeners` confirms **:443 still reports `MTLS: "off"`**. This is the assertion that the human workspace UI was not touched.
3. **The instance security group is unchanged** — port 3000 reachable only from `sg-0cb49292df9f5504e`, no CIDRs. Re-asserted after the apply, not assumed.
4. **N4 fails at the TLS handshake** (no client certificate on :8443) and **N4b returns `403`** on :443 — *not* a Cognito `302`, and *not* a `200`.
5. **N5 returns `200`** with the runtime's `HealthResponse`, using the real leaf on :8443.
6. **N6 fails at the TLS handshake** using a self-signed certificate carrying the **same CN** as the real leaf. **This is the criterion that proves mTLS is real** — without it, a `passthrough` listener would pass 4b and 5 identically while gating on nothing.
7. `pnpm build && pnpm --filter @nevado/runtime test` passes in `nevado-sherpa-tui`, including `sdlc.test.ts` and the prefix integration test.

**What this phase does not prove, and do not claim it does:** nothing about sessions, the ACL, events, or tools. It proves one HTTPS round trip reaches the runtime, that the chain is actually verified, and that three ways of getting it wrong are closed.

**What it no longer needs to prove:** BQ1 and architecture doc U1 are **moot** (§0.9.1) — there is no token and no authorizer in this path. The 2026-09-24 evidence is retained in §0.4 as a platform fact for whoever next needs M2M auth.

### Phase 1 ordering and parallelism

```
1.4 Step 1 (CA + leaf, by hand) ──> 1.4 Step 2/3 (Terraform) ──> 1.5 (targets) ──┐
                                                                                  ├──> 1.7 (negatives)
1.6 (runtime /v1/sdlc namespace — manual SSM deploy) ─────────────────────────────┘
```

- **Tasks 1.1, 1.2, 1.2b and 1.3 are deleted** (§0.9.1). Phase 1 is now four tasks, and the ingress work is Terraform plus one `openssl` session.
- **1.4 Step 1 gates everything** — the trust store cannot be created without the CA certificate in S3, and 1.7 cannot run without the leaf.
- **1.6 (the runtime repo) can start immediately and in parallel**, and should, because its deploy is a **manual SSM RunCommand that kills every in-flight session** and needs an outage window scheduled. It does not depend on the ingress choice — a `/v1/sdlc/health` route is correct either way.
- **1.7 needs 1.4 and 1.5 applied AND 1.6 deployed.** All three negatives hit the runtime.
- **Apply 1.4's rule 6 (:443 deny) together with the :8443 listener**, preferring rule 6 first if they must be split — that ordering fails safe (Task 1.4's rollback note).
...
### Task 2.0 — unforce `createdBy` and `application` in the SDK

**Repurposed.** An earlier draft made this a blocking investigation into how the runtime obtains a caller identity. **BQ4 is now resolved and the investigation is unnecessary:** v1's ACL enforces `application === 'sdlc'`, and `application` is **already settable by a caller** because the forced spread at `session-manager.ts:768` is conditional on `this.application` and the runtime's `SessionManager` is constructed without one (§0.1.d(2)). So nothing in Phase 2 is blocked on this task.

It is still worth doing, in the same 4.0.2 release as Task 2.1, for two reasons.

**Files (in `sherpa-sdk`):**
- `packages/core/src/session-manager.ts` (MOD — two lines)
- the matching test file (MOD)

**The change.** In `create`'s forced block (`session-manager.ts:755-768`), move `createdBy` and `application` above the `...options` spread at `:749` so they act as **defaults** rather than overrides. `id`, `name`, `status`, `publishState`, `transcript` and `tombstone` stay forced — those are genuinely system-owned, and the comment at `:760-764` explains why a caller-supplied `publishState`/`transcript`/`tombstone` would seed a fabricated FSM state.

**Why it is worth two lines:**

1. **It removes a silent-override hazard.** Today `application: 'sdlc'` survives only *because* the runtime's manager happens to have no `application`. Anyone who later adds one — plausibly `'web'`, to make human sessions self-describing — would silently start overriding `'sdlc'` and **quietly disable the entire ACL**, with no test failure anywhere unless Task 2.2's persistence assertion exists.
2. **It populates `createdBy: "m2m:<client_id>"` from day one**, so v2's per-client predicate can be added with no migration and no backfill of existing sessions.

**The objection an earlier draft raised — that this "loosens an invariant the SDK asserts deliberately" — does not survive scrutiny.** `this.userId` is the literal `'dev-user'` (`apps/runtime/src/index.ts:61`) and nothing validates it. The forced block's own comment says it exists *"so a caller cannot forge id/name/status/createdBy/application"*, but in a service with **no authentication at all** (§0.1.b C18) there is no caller to distinguish from another, so the anti-forgery property is vacuous. It becomes real the day identity lands, and on that day the right control is an authenticated hook overwriting the header-derived value — not an un-overridable constant.

**Tests first**, in the SDK's own runner:

| Test name (sentence) | Assertion |
|---|---|
| `it('defaults createdBy to the manager userId when the caller supplies none')` | `create({})` → `createdBy === 'dev-user'`. The regression guard: existing behaviour must not change |
| `it('lets an explicitly supplied createdBy win')` | `create({createdBy: 'm2m:abc'})` → `'m2m:abc'` |
| `it('lets an explicitly supplied application win even when the manager has one')` | manager built with `application: 'web'`; `create({application: 'sdlc'})` → `'sdlc'`. **This is the hazard in (1)** and it is the only test that catches it |
| `it('still forces id, name and status')` | `create({id: 'forged', status: 'completed'})` → a generated `id`, `status: 'active'` |
| `it('still forces publishState, transcript and tombstone to undefined')` | `create({publishState: {...}})` → `undefined` |

**Acceptance:** the SDK's test command passes. Ships in 4.0.2 with Task 2.1.

**Commit (in `sherpa-sdk`):** `fix: let a caller set createdBy and application on session create`

### Task 2.1 — add the three terminal tools, the schema validator and a conformance test

**Files (all in `sherpa-sdk`):**
- `packages/core/src/tool-specs.ts` (MOD)
- `packages/core/src/tool-metadata.ts` (MOD)
- the dispatcher + handler bodies — at HEAD: `packages/core/src/engine/tools/tool-dispatcher.ts` plus a new `packages/core/src/engine/tools/completion-tools.ts`; in 4.0.1: `packages/core/src/agent-engine.ts` (§0.1.b C29, and BQ3 for which version to target)
- `packages/protocol/src/sdlc.ts` (NEW — the payload types, §A.8 P8)
- tests alongside each

**Scope for this task: all three tools, the generic `resultSchema` validator, the double-submit guard, and a spec↔handler conformance test — in ONE 4.0.2 release.** §A.8 P11, P11a, P11b, P11c.

**Why everything goes in one release.** The release mechanism is the binding constraint, not the code (§A.5.0). Each additional release is a manual SSM deploy that stops the service, `rm -rf`s `/opt/sherpa` and loses every in-flight session, human included. So:

| Item | Marginal cost in this release | Cost if deferred |
|---|---|---|
| `reportCompletion` (typed) | — | — |
...
| **Spec↔handler conformance test** (P11a) | one test file | the three new tools join the arrangement that has already drifted twice |

**Deferring `resultSchema` to "later if malformed payloads prove common" is the inverted instinct here.** It costs a window later and zero now.

**Do not add `ajv`.** Neither `sherpa-sdk` nor `nevado-sherpa-tui` has any validation dependency today — verified, zero hits for `ajv|zod|json-schema|jsonschema` across both `packages`/`apps` trees and all four `package.json` files — and `packages/core`'s only non-AWS runtime dependency is `glob`. `ajv` compiles schemas to JavaScript at runtime; doing that on **caller-supplied input** in a **long-lived multi-tenant process** is a worse trade than 120 lines of readable recursive descent over the bounded keyword set in §A.5.3. Hand-rolled is also the house style — every other validation in this codebase is hand-rolled.

**The conformance test will fail on two existing tools, and that is the point.** `saveLearnedPattern`'s handler hard-requires `input.pattern`, which its schema **never declares**, while `content` is declared *required* and never read — **so that tool fails on every schema-compliant call, in shipped code.** `updateTechDebt` declares `{priority, category, location}` while its handler reads `severity`/`file`/`line`, so `severity` silently always defaults (§A.5.0a). Note the first is not a near-miss to be tidied up later: it is a tool that cannot be called successfully. Either fix them or allowlist them with an explicit reason, the way `frontendContractGuard.test.js` handles `KNOWN_GAPS` — **but do not weaken the test to make them pass silently**, because they are the evidence that this whole design is necessary.

Read §A.5.1 for the normative behaviour. The work that is genuinely new, verified absent from `packages/core`:

| Primitive | Status |
|---|---|
| stage all changes | `git add` appears twice in `packages/core/src`, neither as a stage-all for commit |
| commit | **absent.** No porcelain `git commit` anywhere. The only commit-creating code is plumbing `commit-tree` in `git-isolate.ts:228-244`, building two dangling commits to snapshot dirty state, advancing no branch |
| push | **absent.** The only `'push'` string in the package is `git stash push -u -m` at `resume-guard.ts:717` |
| push with retry | absent. `ssmBuildRunner.gitPushWithRetry` (`backend/lambda_handlers/engineeringAgent/ssmBuildRunner.js:438-459`) retries 3× — **read it and port the shape**, it is the only prior art |
| empty-tree detection | absent |
| branch resolution | `git-worktree.ts` reads branch/sha, so the primitive exists; HEAD is not detached on a `commitSha` checkout (§0.1.b C22) so `rev-parse --abbrev-ref HEAD` suffices |
| stderr surfacing | the git runner exists; check what it does with stderr on failure and make sure the message reaches the `toolError` |
| model id in the tool context | **absent.** `ToolContext` carries neither model id nor cumulative usage (§A.5.1) |
| cumulative `TokenUsage` in the tool context | **absent.** Private fields on `AgentEngine` |

**Tests first.** The SDK's own runner — find it in `sherpa-sdk/package.json` / `packages/core/package.json` and match the existing test style in that package.

`packages/core/src/engine/tools/completion-tools.test.ts` (or the 4.0.1 equivalent):

| Test name (sentence) | Fixture | Assertion |
|---|---|---|
| `it('stages, commits and pushes, returning the commit sha it created')` | a temp git repo with one modified file, a fake remote (a bare repo on disk) | result `outcome: 'success'`, `pushed: true`, `commitSha` matches `git rev-parse HEAD` in the repo, `branch` matches `rev-parse --abbrev-ref HEAD`, `filesModified` contains the file |
| `it('refuses a success outcome on an empty tree so the agent can correct itself')` | clean temp repo | a `toolError` whose message says nothing changed; **no commit created** (`git rev-list --count HEAD` unchanged) |
| `it('does not commit or push when the outcome is blocked')` | dirty temp repo, `outcome: 'blocked'`, `blockedReason: 'needs a DB migration'` | `pushed: false`, no new commit, `blockedReason` echoed, `commitSha`/`branch` from current HEAD |
| `it('rejects a blocked outcome with no reason')` | `outcome: 'blocked'`, no `blockedReason` | `toolError` |
| `it('surfaces git stderr when the push is rejected')` | remote with a diverged tip so the push is non-fast-forward | `toolError` whose message contains git's own rejection text (e.g. `non-fast-forward` or `rejected`) — **assert on a substring of real git output, not a message you wrote**, so the test fails if the stderr stops being forwarded |
| `it('retries a failed push three times before surfacing the failure')` | a git runner double that fails twice then succeeds | success; the double was called 3 times. Then: fails all three → `toolError` |
| `it('unions session.filesModified with git name-only')` | session lists `a.ts`; a `runCommand` also changed `pnpm-lock.yaml` | `filesModified` contains both, no duplicates |
| `it('reports the model actually used, not the one requested')` | context carrying a resolved model id different from the requested one | `payload.model` is the resolved one |
| `it('carries cumulative token usage with exactly inputTokens and outputTokens')` | context with totals | `Object.keys(payload.usage).sort()` is `['inputTokens','outputTokens']` — **pin the key set**, because the temptation to add cache counts is exactly what §A.4.3 forbids |
| `it('returns the payload as the tool result so the agent sees what was recorded')` | success case | the `ToolResult` content parses to the same object as the payload |
| `it('fails a success outcome when HEAD is genuinely detached')` | temp repo with `git checkout --detach` | `toolError`, not `branch: 'HEAD'` |
| `it('is blocked in plan mode')` | `buildToolConfig('plan')` | `reportCompletion` is absent from the returned tool list; present in `buildToolConfig('do')` |

That last one is the `PLAN_BLOCKED_TOOLS` change (§A.5.4) and belongs with the tool, not in a separate task.
...
**Implementation:** add the spec to `extendedToolSpecs`, add `reportCompletion` to `PLAN_BLOCKED_TOOLS` (`tool-specs.ts:522` — it is a `Set`), add an entry to `TOOL_METADATA` (`tool-metadata.ts`) — **a tool absent there silently gets `DEFAULT_METADATA`, which feeds the turn loop's parallelism decision**, so an omission here is a real bug that no test will catch unless you write one — and add the dispatch arm plus the handler.

**Acceptance:** the SDK's test command passes, **including the new conformance test**. Then publish 4.0.2 and bump, and confirm in `nevado-sherpa-tui`:
```
cd /Users/jwoolley/dev/nevadoai/nevado-sherpa-tui && pnpm build && \
  node -e "const {getToolConfig}=require('@nevadoai/sherpa-core'); \
    const n=getToolConfig({extended:true,mode:'do'}).tools.map(t=>t.toolSpec.name); \
    console.log(['reportCompletion','submitPlan','submitReview'].every(t=>n.includes(t)))"
```
Expected: `true`. (Adjust the import form for ESM/CJS as the package requires.) Then check plan mode excludes `reportCompletion` and includes `submitPlan`.

**Commits (in `sherpa-sdk`, one release, separate commits):**
- `feat: add reportCompletion tool with git commit and push`
- `feat: add submitPlan and submitReview with opaque payloads`
- `feat: validate terminal tool payloads against a caller-supplied schema`
- `fix: reject a second terminal tool call in the same session`
- `test: bind declared tool schemas to their handlers`
...
**Not blocked on anything.** An earlier draft gated this on a BQ4 investigation; BQ4 is resolved and v1's ACL needs **no SDK change** and **no change to `index.ts`** (§0.4 BQ4, §0.1.d(2)).

**Two independent pieces.**

**(a) The ACL — one predicate.** §A.3. A `preHandler` hook **inside the `/v1/sdlc` encapsulation**, which is its own Fastify scope and therefore cannot leak onto `/v1/workspace/*`. On `GET`, `cancel` and `DELETE` it loads the session and requires **`application === 'sdlc'`**. **Anything else is `404`, never `403`** — a `403` makes the endpoint a session-id oracle.

**Do not implement a `createdBy` predicate.** `createdBy` is forced to the constant `'dev-user'` for every session (`session-manager.ts:759`), so a `createdBy` check would partition nothing while looking like a control — the worst available outcome. Task 2.0 makes the field settable so the value is populated for v2; **the predicate arrives when a second M2M client does.** Write the v1 limitation in the hook's comment.

**Read `X-Nevado-Client-Id` and record it as `createdBy` on create** (Task 2.2), but do not branch on it anywhere.
...
**The tool allowlist** (§A.7.3) is the other half of (b), and it needs a decision: `buildToolConfig` is **subtractive only** and lives in the SDK (§0.1.b C26). Filtering the engine's tool list down to a per-role allowlist therefore either happens in the SDK (`buildToolConfig` gains an allowlist parameter — a release) or is approximated in the runtime by not granting the capability at all. **Recommended: add the allowlist to `buildToolConfig` in the same SDK release as Task 2.1's `reportCompletion`** — it is a small, purely-subtractive change (`filteredTools.filter(t => allow.has(t.toolSpec.name))`) and doing it later means a second release.
...
| `it('does not partition by createdBy in v1, and says so')` | two SDLC sessions with different `createdBy` values | **both readable by either caller.** This test documents the v1 limitation rather than hiding it; when the predicate is added in v2 it is the test that must be inverted, which makes the change discoverable |
| `it('rejects a create from a caller with no client-id header')` | omit the header | `403`. The header is injected by API Gateway with `overwrite:`, so its absence means the request did not come through the intended path |
...
| `it('grants only the allowlisted tools for an engineer session')` | engineer allowlist | the tool list equals §A.7.3's engineer list exactly — **assert the full sorted set, not `toContain`**, because the whole point of an allowlist is that nothing arrives by default |
| `it('does not grant askQuestion to any role')` | each of the three allowlists | `askQuestion` absent from all three |
| `it('does not grant switchMode, searchSessions, saveLearnedPattern or updateTechDebt')` | each allowlist | all four absent. These are the ones that would otherwise arrive by default and each has a specific reason (§A.7.3) |
...
**Acceptance:** `pnpm build && pnpm --filter @nevado/runtime test` passes. Plus, live against testing after deploy: the second M2M client from Phase 1's N2 reads a session created by the first and gets `404`.

**Commit (in `nevado-sherpa-tui`):** `feat: scope SDLC sessions to their creating client and run them unattended`
...
**mTLS makes this simpler than the OAuth version it replaces** (§0.9.1). No token endpoint, no `expires_in`, no 50-minute refresh, no bearer header.

- **Load the cert/key pair once, at module scope**, from the Secrets Manager secret named by `SDLC_CLIENT_CERT_SECRET_ARN`. Build `tls.X509KeyPair(certPEM, keyPEM)` and hold the resulting `*http.Client` for the container's life. **Give the cached pair a TTL** — an hour is ample — so a rotated leaf (§0.9.1: rotate the leaf, no ALB change) is picked up without waiting for a cold start.
- **`crypto/tls` does client certificates natively.** `&http.Client{Transport: &http.Transport{TLSClientConfig: &tls.Config{Certificates: []tls.Certificate{pair}}}}`. **No dependency**, which matters because `packages/core`-style dependency minimalism is the house style here too.
- **No env-var fallback for the certificate.** A `cursorClient.js:49-52`-style local-dev fallback is inappropriate for a credential that grants session creation on an internet-facing listener.
- **Throw a typed error carrying `Status` and `Retryable`.** Callers branch on it: `404` on `GetSession` means the session is gone (Part 2's stall handling fails the cycle); `5xx` and timeouts mean unknown (**never fail a cycle** — §A.2.5); `400` on create means CC built a bad request and retrying will not help.
- **Distinguish a TLS handshake failure from an HTTP error, and do not mark it retryable.** A `tls.CertificateVerificationError` or an `x509` error means the leaf is expired, revoked, or not trusted — retrying cannot fix it, and **it will present as a connection error rather than a status code**, so a naive "no status ⇒ transient" classifier would retry forever. **This is the failure mode a forgotten leaf expiry produces** (§0.9.1), so make its log line say so explicitly.
...
**Tests first.** `backend/go/internal/runtimeclient/client_test.go`. Use `httptest.NewUnstartedServer` with a TLS config requiring and verifying a client certificate — that exercises the real handshake rather than mocking it away, which is the only way to test the one thing mTLS adds.

| Test name (sentence) | Input | Assertion |
|---|---|---|
| `it presents the client certificate on every request` | two `CreateSession` calls against a test server requiring a cert | the server saw a peer certificate on **both** requests, with the expected CN |
| `it loads the certificate once and reuses it` | two calls | the Secrets Manager mock was called **once** |
| `it reloads the certificate after the cache TTL expires` | advance the clock past the TTL | Secrets Manager called twice. **This is what makes leaf rotation work without a redeploy** (§0.9.1) |
| `it returns a non-retryable error when the server rejects the certificate` | test server with a trust store the client cert does not chain to | the error is **not** marked retryable, and its message names certificate verification. **A handshake failure arrives with no HTTP status — a classifier that treats "no status" as transient retries a dead credential forever** |
...
| `it targets port 8443, not 443` | any call | the request URL carries `:8443`. **Omitting the port silently hits :443, where rule 6 returns `403` and the symptom reads as a permissions problem** (Task 1.7) |
...
**Commit:** `feat: add the runtime session client with mTLS`
...
4. **ACL, live, and this is the criterion that matters:** a **human-created** session (one created through `/v1/workspace/sessions`, i.e. with no `application` or with `'web'`) returns **`404`** on `GET /runtime/sessions/{id}`, on cancel, and on `DELETE`. **And `DELETE` on it does not destroy its workspace** — a `404` that still tore down a human's clone would be worse than a `200`. Verified against testing, not only in unit tests.
   - **Per-client isolation is explicitly NOT a criterion.** v1's ACL is `application === 'sdlc'` only (§A.3, BQ4); with one M2M client, `createdBy` partitions nothing. The second M2M client from Phase 1's N2 **can** read the first's sessions, by design, and that must be recorded as a v1 limitation rather than tested for.
...
8. **The three terminal tools are present, one per role, and `PLAN_BLOCKED_TOOLS` excludes `reportCompletion` in plan mode.** Verified against the deployed 4.0.2 with the `getToolConfig` probe in Task 2.1's acceptance.
...
12. **The spec↔handler conformance test passes** (§A.8 P11a) — and the two pre-existing drifts (`saveLearnedPattern`, `updateTechDebt`) are either fixed or allowlisted **with a written reason**, never silently excused.
...
**The one irreversible thing in this phase is the 4.0.2 SDK release** — a published version cannot be withdrawn from GitHub Packages, so a bad `reportCompletion` means publishing a fix, not reverting. Get Tasks 2.0 and 2.1 right before publishing. **This is also why §0.1.d(1)'s collapse had to be decided before Phase 2 and not during it:** a `submitPlan` published with CC's plan schema in its `inputSchema` could not have been taken back.

### Phase 2 ordering and parallelism

```
2.0 (SDK: unforce createdBy/application) ──┐
2.1 (SDK: 3 tools + resultSchema validator) ┴─ ONE 4.0.2 release ──> 2.4 (ACL + profile)
                                                                       │
...
- **2.0 and 2.1 are one SDK release (4.0.2), not two.** They touch different files in the same package; publishing twice costs a second runtime deploy for nothing. **2.0 no longer gates anything** — it was a blocking investigation in an earlier draft and is now a two-line hygiene change (BQ4).
- **2.1 gates 2.4's allowlist half** (`buildToolConfig`'s allowlist filter is §A.8 P12, in the same release) and **2.9** (the smoke test needs `reportCompletion`).
...
- **Each runtime-side merge needs a manual SSM deploy** (§0.3). Batch them: **one** deploy after 2.2 + 2.2b + 2.3 + 2.4 + 2.5, carrying the 4.0.2 bump, rather than five. Remember the all-zeros `/health` pre-flight — the deploy stops the service and `rm -rf`s `/opt/sherpa`, and `SIGTERM` does not drain agent sessions.
- **Phase 0 and Part 2's Phase 4b continue in parallel throughout.**

**A note on what Phase 2's SDK release buys Part 2.** Because all three tools plus the generic `resultSchema` validator ship here (Task 2.1) and none of them carries an SDLC schema, **Part 2 Phases 5 and 6 need no SDK release and no runtime deploy at all** — a plan or review schema change is a `backend/common/sdlcContract.js` edit on CC's normal pipeline. That is the payoff, and it is worth protecting: if a later task wants one more field on a plan or review payload, the answer is a CC-side change to `PLAN_PAYLOAD_CONTRACT`, **never** a sixth outage window.
...
**Tests first.** `backend/__tests__/sdlcContract.test.js`. Header must state what the module is a port of and that `sdlcContract` is the CC-side mirror of `packages/protocol/src/sdlc.ts`.
...
| `it('ignores seq entirely, including a missing or nonsense one')` | `seq: 0`; `seq: '1'`; `seq` absent | `ok: true` for all three. **`seq` is non-durable log decoration (§A.4.1)** — an earlier draft required it to be a positive integer, which would have made a restart's reset counter a poison message |
...
- env: `DYNAMODB_TABLE_NAME`, `SDLC_RUNTIME_API_BASE` (**with `:8443`**), `SDLC_CLIENT_CERT_SECRET_ARN`, and the QA agent ARN if `routeAfterEngineering`'s `ai-qa` branch invokes it. **No M2M or token vars** — §0.9.1 removed them
...
- `apps/runtime/src/ws/broadcaster.ts` (MOD — the hook)
- `apps/runtime/package.json` (MOD — `@aws-sdk/client-sqs`)

**This is the largest runtime-side task in Part 1.** Four independent pieces: the seq counter, the event filter, the two-tier cadence, and the publish call.
...
`seq` survives in the envelope only as an **in-memory, per-session, non-durable counter for log readability** (§A.4.1). It resets on restart and nothing reads it. **If retaining it tempts anyone to depend on it, drop it** — no test and no consumer in this spec reads it.
...
**(d) Two protocol additions ride this task**, both from §A.8: `reason?: PauseReason` on the `sessionStatus` frame (P4/P5) and `'queued'` on `SessionStatus` (P2). Both are SDK changes (BQ3's release choreography). **`'queued'` needs an exhaustiveness audit** across all three repos before merging — a widened union with a non-exhaustive consumer is a silent fallthrough, which is exactly the `mapStatusToStageIndex` failure class (`CycleProgress.jsx:29-31`). `runtimeEvents.mapSessionEvent`'s `default` arm (`:95-99`) already tolerates unknown types so CC is safe; the human surfaces may not be.
...
**Deploy:** publish/bump the SDK for (d), then the manual SSM RunCommand (`nevado-sherpa-tui/docs/AWS_DEPLOYMENT.md:578-610`) with the **all-zeros `/health` pre-flight** (`:553-576`). **Batch this with Task 3.7's manual systemd edit** — same outage window, and the publisher is inert without the queue URL.
...
1. **Part 2 §1.10 states that the runtime repo is not checked out and marks every runtime claim ⚠ VERIFY.** It is checked out, at **`/Users/jwoolley/dev/nevadoai/nevado-sherpa-tui`**, and the SDK is a third repo at `/Users/jwoolley/dev/nevadoai/sherpa-sdk` (§0.0). Every runtime-side claim in this document is verified against real code. **Anyone reading Part 2 alone will believe OD-4 is still open**; a pointer at the top of its §1.10 fixes that.
