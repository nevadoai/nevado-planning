...
|---|---|
| **Ingress authentication** | Two options behind `var.sdlc_ingress_mode`. **`token` is what runs** — a bearer token the runtime verifies itself, generated by Terraform with no human step. `mtls` is retained and inactive. mTLS was declined on **cost**, not feasibility; see Part 1 §0.9.1. |
| **SDK changes** | **None.** Changing `@nevadoai/sherpa-core` was explicitly authorised and declined on merit: `'completed'` is a human complete/archive flag, and emitting it from the engine loop would break `nevado-sherpa-ide`'s `/resume`. No session status can distinguish a session that finished having submitted a result from one that finished having submitted nothing — delivery is not a lifecycle state. |
