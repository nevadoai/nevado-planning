...
## Cutover as actually implemented, 2026-09-26

The phases below were built. **The operator sequence for turning the runtime path on is this, and it is not what any earlier draft described:**

1. **Set the ingress mode durably.** `gh variable set TESTING_SDLC_INGRESS_MODE --repo nevadoai/command-center --body token`. Then dispatch **Deploy to Testing** once. Terraform generates the bearer token itself — `random_password` plus a secret version — so there is **no CA, no certificate, and no manual credential step**.
2. **Deploy the runtime artifact and set `SDLC_AUTH_TOKEN_ARN` on its systemd unit in the same restart window.** Cloud-init writes that unit on **first boot only**, so a Terraform change to the variable never reaches a running instance; the unit must be rewritten over SSM. Read the ARN from the `sdlc_auth_token_secret_arn` Terraform output rather than retyping it — in any mode but `token` the output is null, which is the signal that the ingress is not applied. A restart **kills every live session**, so check `/health` reports all three counters at zero first.
3. **Confirm the runtime registered the namespace.** The journal must show `SDLC routes registered at /v1/sdlc`. If it shows `SDLC_AUTH_TOKEN_ARN is set but the secret could not be read`, the routes are **withheld entirely** — deliberately, so a check that can never pass is not served. That is the symptom of a missing or destroyed secret, not of an IAM fault; an IAM fault reads as `AccessDenied`.
4. **Set `agentConfig.buildMode: 'runtime'` on one application.** A single DynamoDB field; rollback is removing it.
5. **Run a queue-path baseline cycle, then a runtime cycle, then diff the records.**

**Known blocker on step 1 until `command-center#858` merges.** `sdlc_ingress_mode` was only a `workflow_dispatch` input and was persisted nowhere, so every merge to `develop` re-applied with `none` and destroyed the secret, its version and the ALB rule — and the reverting apply reported **success**. Observed live: applied 13:12, destroyed 13:40 by an unrelated merge, and a runtime deployed at 13:45 then withheld its routes because the secret it was pointed at no longer existed.

**Scope caveat.** Only **human-approved** cycles reach the Go path. PM auto-approval still calls the JavaScript `approvePlan` in-process, whose `runtimeDispatch.js` makes no network call and returns `stub-<uuid>`, so a PM-linked runtime cycle still parks. Pre-existing, not a regression, and not fixed by any phase below.
...
|---|---|---|
| **The runtime repo exists and every runtime claim is now verified.** Revision 1 said it did not and marked runtime items “⚠ VERIFY”. It is `/Users/jwoolley/dev/nevadoai/nevado-sherpa-tui`; the SDK is a third repo at `/Users/jwoolley/dev/nevadoai/sherpa-sdk` | §1.10, rewritten | R§4(m) |
...
- **The ACL** (A§2.2, Part 1 §A.3). **v1 scopes on `application === 'sdlc'`**, enforced on every `/v1/sdlc` read / cancel / delete — that is machine/human separation, which is the whole of v1's real security value. `createdBy: "m2m:<client_id>"` is written because it is free, but **per-client partitioning is deferred** until a second M2M client exists. **Part 1 owns the mechanism and is authoritative on it** — revision 2 of this document asserted it required a two-line SDK change unforcing `createdBy` and `application` in `SessionManager.create`; that is no longer the case. Do not re-derive it here; read Part 1 §A.3.
...
- **The event transport** (A§2.4, A§2.5) — the FIFO queue, the envelope, the two-tier publish cadence, `sdlcEventConsumer`, and the conditional status write that makes replay safe. **`seq` is no longer durable state:** the FIFO `MessageDeduplicationId` is a **per-envelope UUID v4** minted at envelope construction and reused across retries of that one `SendMessage`; ordering remains `messageGroupId = sessionId` (R§2.4, R§7.2). Risk R5c is deleted as a consequence.
- **The terminal tools** (A§2.6, Part 1 §A.5). **Three, named, with two of them carrying an opaque payload** (D§ verdict):
  - **`reportCompletion` stays fully typed.** Branch, commit sha, files changed are git and session vocabulary, and its side effect — stage, commit, push — needs the working tree. Keeping it typed also removes a round trip: `commitSha`/`branch` are absent from the event stream today, so `buildCompletionEffect` has to fetch the `AgentSession` for them (`runtimeEvents.js:20-23`, `:88-90`).
...
- **SDK version: patch 4.0.1 and publish 4.0.2.** Not HEAD (R§2.3, R§7.3). The deployed runtime declares `"@nevadoai/sherpa-core": "^4.0.1"` (`nevado-sherpa-tui/apps/runtime/package.json:22`). Every line reference to SDK source in this document is to **4.0.1**, read from `nevado-sherpa-tui/node_modules/.pnpm/@nevadoai+sherpa-core@4.0.1/node_modules/@nevadoai/sherpa-core/src/`.
- **No phase in this document requires an SDK release or a runtime deploy. Confirmed after the revision-3 rework.** All three tools, `resultSchema`, its validator and the "already submitted" guard ship together in Part 1 Task 2.1's **single** 4.0.2 release — the release that is being cut anyway. The generic `payload` plus `resultSchema` makes every subsequent plan-schema and QA-schema change a Command Center edit on CC's normal pipeline. D§1d puts the economics precisely: **`resultSchema`'s marginal cost over an opaque-payload-only design is zero deploy windows now, and one deploy window if deferred** — so the seam goes in on the release already being taken. This is the operational payoff of the `submitResult` decision (R§2.1) and it is worth stating as a property rather than leaving implicit: **after Part 1 Phase 3, nothing here waits on a runtime deploy window.** If a task below appears to need one, that is a design error in the task rather than a scheduling problem — raise it.
...
### 1.10 Runtime-side: the repo exists, and the claims are verified
...
| Repo | Path | Role |
|---|---|---|
| `command-center` | `/Users/jwoolley/dev/nevadoai/command-center` | Cycles, steps, roles, tenants. This document's subject |
| **`nevado-sherpa-tui`** | `/Users/jwoolley/dev/nevadoai/nevado-sherpa-tui` | **The runtime server.** `apps/runtime` is the HTTP service; `apps/tui` and `apps/web` are the human surfaces. This is what revision 1 could not find, because it is not named `nevado-sherpa-tui` |
| `sherpa-sdk` | `/Users/jwoolley/dev/nevadoai/sherpa-sdk` | `@nevadoai/sherpa-core` and `@nevadoai/sherpa-protocol`. `develop` HEAD is 64 commits ahead of the deployed `v4.0.1` |

**Read 4.0.1, not HEAD.** `nevado-sherpa-tui/apps/runtime/package.json:22` declares `"@nevadoai/sherpa-core": "^4.0.1"`, and per §0.2 the decision is to patch 4.0.1 and publish 4.0.2 rather than bump 64 commits. The deployed source is on disk at `nevado-sherpa-tui/node_modules/.pnpm/@nevadoai+sherpa-core@4.0.1/node_modules/@nevadoai/sherpa-core/src/`. Every SDK line reference below is to that tree.
...
| **`commitSha` and `baseBranch` are unreachable from the create path** | `cloneWorkspace` takes `(sessionId, repoUrl, branch?)` only (`workspace.ts:33`). The sole mention of `commitSha` in `apps/runtime/src` outside tests is a comment about the resume settle strategy (`sessions.ts:233`) | Part 1 Task 2.3 builds it. Phase 6 depends on that task, not on existing behaviour |
...
| `DEP-P1-10` | **new.** Part 1 Task 2.3 must make a failed `checkoutCommit` a create-time **`400`**, not a `201`. `checkoutCommit` returns `false` and the session proceeds on the branch tip (§1.11b). Without this, Task 6.3's independent verification is the only thing standing between a bad sha and a confident review of the wrong code | 6.3 |
...
| 2 | `:1849` | `cycle.branch = engineeringResult.branchName` |
...
**Why (A§4.3).** *"A reviewer that can edit the code under review will fix findings instead of reporting them, and the independence that justified running the review is gone."* The `qa` allowlist is `readFile`, `listFiles`, `searchCode`, `runCommand`, `updateTaskTracker`, `dispatchSubAgents`, and **`submitResult`** — not `editFile`, `writeFile`, or `reportCompletion`.

**Revision 3 — the terminal tool is `submitReview` again.** Revision 2 renamed it `submitResult`; revision 3 restores the three named tools (§0.2, D§ verdict). A§4.3's own allowlist table remains wrong on the other axis: it lists `reportCompletion` for the `qa` role, which would be a disaster — `reportCompletion` commits and pushes, so granting it to a reviewer hands write access to the very session that must not have it, and the empty-tree rule would then reject every honest QA completion anyway. Part 1 caught this and corrects it (R§4(h)); follow Part 1.
...
**Commit.** None, unless the verification finds a gap. If it does, the fix is in `sherpa-sdk` / the runtime server and belongs to Part 1.
...
1. **`gitPushWithRetry` (`:438-459`)** retries three times, and A§2.6 says that behaviour is *"worth preserving"* in `reportCompletion` — but the final failure must reach the agent, not be swallowed. `reportCompletion` is Part 1's and lives in `sherpa-sdk`, which has **no push primitive at all** (§1.10). So: before deleting, copy the retry logic and its backoff into the Part 1 issue as reference, or the knowledge is lost with the file.
...
Rollout order: one application in testing → the same application for ten cycles (Phase 4 exit criterion 1) → remaining testing applications → production applications one at a time.
...
4. A human approves. `planSessionId`'s session 404s; `engineeringSessionId` is a new, active session on the cycle's branch.
5. Activity feed fills. The cycle stays in `engineering` past fifteen minutes without being failed.
6. `reportCompletion` → `building`, with `cycle.branch`, `cycle.githubUrl`, `cycle.buildHeadSha`, and a **draft PR** whose `headRefOid` equals the session's `commitSha` and whose body is the session's `summary` + `verification`.
...
| **R11 / R12 — the static shared secret and the forgeable client-id header** | Part 1 Phase 1 | Not this document's, but Task 4.5 adds a second consumer of the M2M path (`stuckCycleDetector`), which widens the blast radius of a leaked secret by one Lambda. Note it; do not design around it here |
...
A§7 U2. Verified in `sherpa-sdk`: `onPlanCreated` exists in `EngineCallbacks` (`packages/core/src/types.ts:110`) and `EngineEvents` (`engine/engine-types.ts:32`), is fired by `executeWritePlan` (`workspace-tools.ts:219`), and `planCreated` is a real `ServerMessage` carrying `{sessionId, filePath, content}` (`messages.ts:22`). Whether the runtime server forwards it to the publisher is Part 1's to determine (`nevado-sherpa-tui` is now readable — §1.10 — so it is checkable, just not this document's). **It does not matter:** Task 5.1 explicitly maps `planCreated` to a progress activity and **not** to `expandedRequirements` — the contract is the `submitResult` payload (A§3.1: *"Do not ask the planner to write markdown and then parse it back"*). If `planCreated` never arrives, the only loss is one activity line.
...
Two items, both outside this document's authority, both worth naming so they are not mistaken for settled.
...
The decision is the runtime owner's, not this document's. **What this document depends on:** if the answer is no, then the `submitResult` collapse (§0.2, §1.7) moves from "decided" to "load-bearing", because it becomes the only mechanism that keeps plan-schema and QA-schema iteration out of outage windows. It is already decided, so this is not a blocker — but if anyone proposes reverting to typed `submitPlan`/`submitReview`, the pipeline question must be answered first.
...
| **7.2** `ssmBuildRunner.js` | **Survives as a delete**, unchanged. Its two harvests still matter: `gitPushWithRetry`'s retry logic for `reportCompletion`, and recording `COMMAND_TIMEOUT_SECONDS = 300` against the runtime's 120s cap (R1) before the file is gone |
...
**`BQ-GO-2` — do the runtime-facing HTTP client and SQS consumer become Go too?** They are Part 1's, but they are Lambdas, so the mandate reaches them. If `sdlcEventConsumer` is Go, then `runtimeEvents.mapSessionEvent` — which Tasks 4.4, 5.1 and 6.5 all extend — is Go, and the existing JS mapper plus its two test suites (`runtimeEvents.test.js`, `applyRuntimeEffect.test.js`, both landed in `546408af`) are superseded rather than extended. That is a real loss of working, tested code and it should be an explicit decision rather than a side effect.
